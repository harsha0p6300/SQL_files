...
#Find the total revenue (quantity × price) generated by each product, sorted highest to lowest.
select product_id,product_name,total_revenue
from(
	select p.product_id,p.product_name,sum(oi.quantity*oi.unit_price-oi.discount_amount) as total_revenue
    from products p
